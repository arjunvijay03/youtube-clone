...
function HomeLeft({active}) {
  ...
  return (
    ...
    <div className='w-100 p-2 px-3  d-flex flex-column gap-2 overflow-auto hide-scroll-bar' style={{height:'calc(100vh - 60px)'}}> {/*hide scroll bar class is costom css*/ }
        <div onClick={()=>handleClick('')} className={`${active === '' && 'bg-dark'}  w-100 p-2  home-left-options text-light rounded-3 d-flex align-items-center gap-4 fs-6 fw-fw-normal ps-3 `}  role='button'>  <Icon.HouseFill size={20}></Icon.HouseFill>Home</div>
        <div onClick={()=>handleClick('trending')} className={` ${active === 'trending' && 'bg-dark'}  w-100 p-2  home-left-options text-light rounded-3 d-flex align-items-center gap-4 fs-6 fw-fw-normal ps-3  `} role='button'>  <Icon.Fire size={20}></Icon.Fire>Trending</div>
        <div onClick={()=>handleClick('music')} className={` ${active === 'music' && 'bg-dark'}  w-100 p-2  home-left-options text-light rounded-3 d-flex align-items-center gap-4 fs-6 fw-fw-normal ps-3  `} role='button'>  <Icon.MusicNoteBeamed size={20}></Icon.MusicNoteBeamed>Music</div>
        <div onClick={()=>handleClick('gaming')} className={` ${active === 'gaming' && 'bg-dark'}  w-100 p-2  home-left-options text-light rounded-3 d-flex align-items-center gap-4 fs-6 fw-fw-normal ps-3  `} role='button'>  <Icon.Controller size={20}></Icon.Controller>Gaming</div>
